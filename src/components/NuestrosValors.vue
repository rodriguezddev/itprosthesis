<template>
    <div data-aos="zoom-out-up" class="contValores" style="">
        <h2 class="nuestros col-md-5">NUESTROS VALORES</h2>
        <div class="row  contEmpoderamosTextoValores">
            <div class="col-md-5 col-lg-3  row flex-column align-items-center m-0 pl-4 pr-4 pt-4" @mouseover="agU()" @mouseout="quite()" :class=" [showU ? 'colorU' : '']">
                <div class="borderCIr row justify-content-center align-items-center">
                    <div class="denC colorUno row justify-content-center align-items-center">
                        <svg class="icono-tas" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 211 211"><defs></defs><g id="Capa_2" data-name="Capa 2"><g id="Capa_4" data-name="Capa 4"><polyline class="check" points="51.96 113.05 95.47 148 170 57.94"/><circle class="check" cx="105.5" cy="105.5" r="103"/></g></g></svg>
                    </div>
                </div>
                <div class="row flex-column align-items-center m-0">
                    <div class=" bordeEtiquU mt-4 mb-4">
                        <h4 class="text-center colorUno p-2 m-1">ACCESIBILIDAD</h4>
                    </div>
                    <p class="textoVal">Todas nuestras propuestas son formuladas con el fin de que todos puedan acceder a nivel económico, social y cultural</p>
                </div>
            </div>
            <div class="col-md-5 col-lg-3  row flex-column align-items-center m-0 pl-4 pr-4 pt-4" @mouseover="agD()" @mouseout="quite()" :class=" [showD ? 'colorD' :  '' ]">
                <div class="borderCIr row justify-content-center align-items-center">
                    <div class="denC colorDos row m-0 justify-content-center align-items-center">
                        <div class="border-interno">
                            <svg class="icono-tas" version="1.1" id="Capa_1" width="6rem" height="6rem" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                                viewBox="0 0 482.13 482.13" style="enable-background:new 0 0 482.13 482.13;" xml:space="preserve">
                                <g>
                                <g>
                                    <path d="M475.266,126.603c-0.001,0-0.001,0-0.002,0l-0.016-0.016l-31.12-4.44c-4.374-0.623-7.415-4.674-6.792-9.048
                                        c0.299-2.102,1.422-3.998,3.12-5.272l25.152-18.864c3.535-2.651,4.251-7.665,1.6-11.2l-28.8-38.4
                                        c-2.651-3.535-7.665-4.251-11.2-1.6l-25.144,18.864c-3.52,2.671-8.538,1.983-11.209-1.537c-1.297-1.709-1.853-3.868-1.543-5.991
                                        l4.448-31.12c0.627-4.374-2.41-8.427-6.783-9.055c-0.003,0-0.006-0.001-0.009-0.001L339.48,2.139
                                        c-4.373-0.628-8.428,2.408-9.056,6.782c0,0.001,0,0.001,0,0.002l-4.448,31.12c-0.623,4.374-4.674,7.415-9.048,6.792
                                        c-2.102-0.299-3.998-1.422-5.272-3.12L292.8,18.563c-2.651-3.535-7.665-4.251-11.2-1.6l-24.04,18.04
                                        c-10.876-1.156-21.844-1.156-32.72,0C153.614,43.144,97.759,99.946,90.816,171.299c-0.216,2.264-0.296,4.512-0.408,6.76h-2.4
                                        c-3.796,0.007-7.065,2.681-7.824,6.4l-5.528,26.288l-22.144-15.272c-3.178-2.193-7.47-1.802-10.2,0.928l-24,24
                                        c-2.726,2.729-3.116,7.015-0.928,10.192l15.2,22.104l-26.296,5.528c-3.678,0.806-6.296,4.067-6.288,7.832v32
                                        c-0.002,3.784,2.649,7.052,6.352,7.832l26.296,5.528l-15.2,22.104c-2.188,3.177-1.798,7.463,0.928,10.192l24,24
                                        c2.73,2.73,7.022,3.121,10.2,0.928l22.104-15.24l5.528,26.296c0.759,3.719,4.028,6.393,7.824,6.4h32
                                        c3.784,0.002,7.052-2.649,7.832-6.352l5.528-26.296l22.104,15.2c3.177,2.188,7.463,1.798,10.192-0.928l12.424-12.464v22.8
                                        c0.036,11.381,6.133,21.88,16,27.552v44.448c0,17.673,14.327,32,32,32h32c17.673,0,32-14.327,32-32v-44.448
                                        c9.867-5.672,15.964-16.171,16-27.552v-31.664c0.217-13.323,7.187-25.623,18.504-32.656c6.639-4.307,12.935-9.122,18.832-14.4
                                        l-10.672-11.92c-5.275,4.728-10.911,9.037-16.856,12.888c-15.885,9.97-25.609,27.334-25.808,46.088v31.664c0,8.837-7.163,16-16,16
                                        h-64c-8.837,0-16-7.163-16-16v-32.328c-0.185-18.399-9.756-35.428-25.376-45.152c-63.407-39.842-82.511-123.541-42.669-186.948
                                        c24.72-39.341,67.862-63.286,114.325-63.452c-0.119,0.285-0.224,0.576-0.312,0.872c-0.297,2.1,0.253,4.233,1.528,5.928
                                        l18.856,25.144c1.988,2.573,2.179,6.107,0.48,8.88c-1.636,2.79-4.799,4.312-8,3.848l-31.12-4.448
                                        c-4.374-0.627-8.427,2.41-9.055,6.783c0,0.003-0.001,0.006-0.001,0.009l-6.792,47.512c-0.627,4.373,2.41,8.427,6.783,9.055
                                        c0.003,0,0.006,0.001,0.009,0.001l31.12,4.448c4.374,0.623,7.415,4.674,6.792,9.048c-0.299,2.102-1.422,3.998-3.12,5.272
                                        l-25.144,18.856c-3.535,2.651-4.251,7.665-1.6,11.2l28.8,38.4c2.651,3.535,7.665,4.251,11.2,1.6l25.152-18.856
                                        c3.538-2.646,8.552-1.923,11.198,1.616c1.266,1.693,1.811,3.819,1.514,5.912l-4.44,31.12c-0.628,4.373,2.408,8.428,6.782,9.056
                                        c0.001,0,0.001,0,0.002,0l47.52,6.792c0.374,0.048,0.751,0.072,1.128,0.072c3.983,0.005,7.362-2.921,7.928-6.864l4.44-31.12
                                        c0.623-4.374,4.674-7.415,9.048-6.792c2.102,0.299,3.998,1.422,5.272,3.12l18.864,25.144c2.651,3.535,7.665,4.251,11.2,1.6
                                        l38.4-28.8c3.535-2.651,4.251-7.665,1.6-11.2l-18.856-25.144c-1.979-2.577-2.17-6.105-0.48-8.88c1.601-2.827,4.793-4.359,8-3.84
                                        l31.12,4.44c4.373,0.628,8.428-2.409,9.056-6.782c0-0.001,0-0.001,0-0.002l6.784-47.52
                                        C482.676,131.285,479.64,127.231,475.266,126.603z M210.112,410.059h64v16h-64V410.059z M210.112,442.059h64v8
                                        c0,8.837-7.163,16-16,16h-32c-8.837,0-16-7.163-16-16V442.059z M146.424,304.099c4.368,3.572,8.932,6.898,13.672,9.96
                                        c7.559,4.777,13.249,12.004,16.12,20.472l-17.16,17.128l-16.616-11.456c-7.276-5.014-17.239-3.18-22.254,4.096
                                        c-1.202,1.745-2.046,3.711-2.482,5.784l-4.192,19.976H94.496l-4.192-19.944c-1.82-8.647-10.305-14.182-18.952-12.362
                                        c-2.073,0.436-4.039,1.28-5.784,2.482l-16.624,11.424l-14.576-14.544l11.448-16.608c5.019-7.273,3.191-17.237-4.082-22.256
                                        c-1.746-1.205-3.714-2.051-5.79-2.488L16,291.563v-19.008l19.944-4.2c8.644-1.833,14.166-10.326,12.333-18.971
                                        c-0.435-2.051-1.268-3.996-2.453-5.725l-11.456-16.656l14.576-14.544l16.632,11.456c7.279,5.009,17.242,3.169,22.251-4.11
                                        c1.2-1.743,2.042-3.707,2.477-5.778l1.4-6.656c1.282,9.218,3.405,18.298,6.344,27.128c-26.323,3.138-45.118,27.022-41.98,53.345
                                        c3.138,26.323,27.022,45.118,53.345,41.98C125.259,327.934,139.13,318.293,146.424,304.099z M134,292.827
                                        c-5.974,16.605-24.277,25.223-40.882,19.25c-16.605-5.974-25.223-24.277-19.25-40.882c4.57-12.703,16.631-21.164,30.131-21.136
                                        c0.104,0,0.2,0,0.304,0C111.66,265.922,121.707,280.391,134,292.827z M460.512,172.971l-23.2-3.312
                                        c-13.122-1.874-25.278,7.245-27.152,20.366c-0.9,6.301,0.74,12.702,4.56,17.794l14.056,18.744l-25.6,19.2l-14.064-18.736
                                        c-7.952-10.605-22.995-12.756-33.599-4.804c-5.092,3.818-8.459,9.503-9.361,15.804l-3.24,23.232l-31.672-4.528l3.312-23.2
                                        c1.879-13.121-7.235-25.281-20.356-27.159c-6.305-0.903-12.709,0.737-17.804,4.559l-18.744,14.056l-19.2-25.6l18.736-14.064
                                        c10.605-7.952,12.756-22.995,4.804-33.599c-3.818-5.092-9.503-8.459-15.804-9.361l-23.2-3.312l4.528-31.672l23.2,3.312
                                        c13.121,1.879,25.281-7.235,27.159-20.356c0.903-6.305-0.737-12.709-4.559-17.804l-14.104-18.76l25.6-19.2l14.064,18.744
                                        c7.952,10.605,22.995,12.756,33.599,4.804c5.092-3.818,8.459-9.503,9.361-15.804l3.312-23.2l31.672,4.52l-3.312,23.2
                                        c-1.874,13.122,7.244,25.278,20.366,27.152c6.301,0.9,12.702-0.74,17.794-4.56l18.744-14.056l19.2,25.6l-18.744,14.064
                                        c-10.605,7.952-12.756,22.995-4.804,33.599c3.818,5.092,9.503,8.459,15.804,9.361l23.2,3.312L460.512,172.971z"/>
                                </g>
                                </g>
                                <g>
                                    <g>
                                        <path d="M344,76.171c-35.346,0-64,28.654-64,64c0,35.346,28.654,64,64,64c35.346,0,64-28.654,64-64
                                            C407.96,104.841,379.33,76.21,344,76.171z M344,188.171c-26.51,0-48-21.49-48-48s21.49-48,48-48c26.51,0,48,21.49,48,48
                                            C391.969,166.668,370.497,188.14,344,188.171z"/>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="234.112" y="354.059" width="16" height="24"/>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <path d="M207.12,275.491c-49.385-19.292-73.78-74.966-54.487-124.351c7.476-19.136,20.893-35.369,38.279-46.313l-8.552-13.536
                                            c-52.336,32.971-68.035,102.126-35.064,154.462c12.769,20.269,31.701,35.91,54.016,44.626c19.752,7.568,32.795,26.528,32.8,47.68
                                            h16C250.137,310.307,233.035,285.418,207.12,275.491z"/>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="394.112" y="298.059" width="56" height="16"/>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="402.113" y="334.822" transform="matrix(0.6401 -0.7683 0.7683 0.6401 -133.6437 446.8239)" width="16" height="62.48"/>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="346.112" y="362.059" width="16" height="56"/>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="9.128" y="130.058" transform="matrix(0.9701 -0.2425 0.2425 0.9701 -32.2251 14.3351)" width="65.968" height="15.992"/>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="46.111" y="35.657" transform="matrix(0.5812 -0.8137 0.8137 0.5812 -34.3557 73.3688)" width="15.992" height="68.816"/>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="97.938" y="1.024" transform="matrix(0.9688 -0.2477 0.2477 0.9688 -5.133 27.2971)" width="16" height="66.056"/>
                                    </g>
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                                <g>
                                </g>
                            </svg>
                        </div>
                    </div>
                </div>
                <div class="row flex-column align-items-center m-0">
                    <div class=" bordeEtiquD mt-4 mb-4">
                         <h4 class="text-center colorDos p-2 m-1">INNOVACiÓN</h4>
                    </div>
                    <p class="textoVal">Enfatizamos las necesidades de nuestros pacientes para desarrollar soluciones rentables de valor, que logren ser sostenibles en el largo plazo.</p>
                </div>
            </div>
            <div class="col-md-5 col-lg-3 row flex-column align-items-center m-0 pl-4 pr-4 pt-4" @mouseover="agT()" @mouseout="quite()" :class=" [showT ? 'colorT' : '' ]">
                <div class="borderCIr row justify-content-center align-items-center">
                    <div class="denC colorTres row justify-content-center align-items-center">
                        <div class="border-interno">
                            <svg class="icono-tas" height="6rem"  width="6rem" viewBox="-78 0 1166 1166.2525" xmlns="http://www.w3.org/2000/svg"><path d="m979.617188 298.582031c-79.675782-171.316406-246.75-285.257812-435.328126-296.875l-18.746093-1.707031-17.132813.425781c-58.191406 2.140625-115.21875 16.882813-167.140625 43.25-138.039062 69.011719-225.046875 210.304688-224.535156 364.632813v72.363281l-113.828125 189.71875c-3.613281 5.992187-3.710938 13.476563-.261719 19.570313 3.457031 6.09375 9.921875 9.859374 16.925781 9.859374h121.644532l-11.792969 175.191407c-8.996094 51.820312 25.15625 101.300781 76.808594 111.285156 1.273437.25 2.585937.386719 3.878906.386719.695313 0 1.402344-.039063 2.078125-.109375l118.324219-12.835938v173.078125c0 10.730469 8.699219 19.4375 19.4375 19.4375h466.425781c10.734375 0 19.433594-8.707031 19.433594-19.4375v-193.136718l151.855468-375.230469c33.367188-90.863281 30.476563-191.074219-8.046874-279.867188zm-28.273438 266.035157-152.996094 378.132812c-1.0625 2.773438-1.457031 5.742188-1.179687 8.699219 0 .40625-.230469.761719-.230469 1.167969v174.761718h-427.554688v-174.910156c0-.402344-.21875-.761719-.234374-1.164062-.019532-.40625.167968-.855469.109374-1.292969-.222656-1.113281-.582031-2.191407-1.023437-3.230469-.210937-.855469-.488281-1.6875-.8125-2.507812-.785156-1.652344-1.796875-3.195313-3.027344-4.5625-.199219-.359376-.417969-.703126-.648437-1.039063-.214844-.171875-.445313-.328125-.664063-.515625-4.027343-3.960938-9.597656-5.929688-15.222656-5.382812l-136.769531 14.800781c-30.167969-7.609375-49.097656-37.496094-43.09375-68.023438l13.414062-197.828125c.753906-10.714844-7.324218-20.015625-18.035156-20.769531-1.460938 0-109.414062 0-109.414062 0l98.914062-164.957031c.183594-.398438.328125-.808594.449219-1.234375.566406-1.175781 1-2.410157 1.304687-3.667969.382813-1.273438.648438-2.582031.773438-3.898438 0-.445312.265625-.835937.265625-1.296874v-77.589844c-.53125-139.675782 78.132812-267.597656 203.039062-330.144532 46.894531-23.8125 98.398438-37.148437 150.941407-39.105468h15.546874c180.457032 5.09375 342.613282 111.585937 418.96875 275.175781 34.398438 79.453125 36.976563 169.09375 7.179688 250.382813zm0 0"/><path d="m544.289062 200.621094c-71.515624-60.757813-178.234374-54.257813-241.835937 14.722656-63.617187 68.992188-61.449219 175.875 4.890625 242.230469l219.855469 219.90625c4.484375 4.476562 10.855469 6.484375 17.089843 5.40625 1.101563.191406 2.21875.289062 3.34375.289062 5.152344 0 10.089844-2.054687 13.742188-5.695312l219.855469-219.90625c66.339843-66.355469 68.511719-173.238281 4.894531-242.230469-63.613281-68.980469-170.335938-75.480469-241.835938-14.722656zm209.464844 229.476562-209.464844 209.488282-209.46875-209.488282c-53.132812-53.125-53.132812-139.261718 0-192.398437 53.128907-53.125 139.277344-53.125 192.398438 0l3.351562 3.34375c7.582032 7.589843 19.890626 7.589843 27.484376 0l3.335937-3.34375c53.128906-53.125 139.265625-53.125 192.398437 0 53.125 53.136719 53.125 139.273437 0 192.398437zm0 0"/><path d="m431.023438 217.316406c-47.160157 0-89.675782 28.398438-107.726563 71.976563-18.046875 43.574219-8.070313 93.722656 25.265625 127.0625 7.625 7.359375 19.746094 7.261719 27.246094-.230469 7.488281-7.503906 7.601562-19.617188.238281-27.25-22.230469-22.230469-28.878906-55.660156-16.847656-84.710938 12.042969-29.050781 40.386719-47.988281 71.824219-47.980468 10.730468 0 19.433593-8.703125 19.433593-19.4375 0-10.722656-8.703125-19.429688-19.433593-19.429688zm0 0"/></svg>
                        </div>
                    </div>
                </div>
                <div class="row flex-column align-items-center m-0">
                    <div class=" bordeEtiquT mt-4 mb-4">
                        <h4 class="text-center colorTres p-2 m-1">PASIÓN</h4>
                    </div>
                    <p class="textoVal">Somos un grupo de 10 personas apasionadas y comprometidas con la idea de vivir en un mundo más justo para todos. </p>
                </div>
            </div>
            <div class="col-md-5 col-lg-3 row flex-column align-items-center m-0 pl-4 pr-4 pt-4" @mouseover="agC()" @mouseout="quite()" :class=" [showC ? 'colorC' : '' ]">
                <div class="borderCIr row justify-content-center align-items-center">
                    <div class="denC colorCuatro row justify-content-center align-items-center">
                        <div class="border-interno">
                            <svg class="icono-tas" id="Icons" height="6rem"  width="6rem" viewBox="0 0 74 74"  xmlns="http://www.w3.org/2000/svg"><path d="m30.7 31.3h-26a1 1 0 0 1 -1-1v-26a1 1 0 0 1 1-1h26a1 1 0 0 1 1 1v26a1 1 0 0 1 -1 1zm-25-2h24v-24h-24z"/><path d="m71 31.3h-28.6a1 1 0 0 1 -.876-1.482l14.3-26a1.04 1.04 0 0 1 1.752 0l14.3 26a1 1 0 0 1 -.876 1.482zm-26.907-2h25.216l-12.609-22.922z"/><path d="m25.06 71h-14.715a1 1 0 0 1 -.886-.536l-7.345-14a1 1 0 0 1 .033-.986l7.345-12a1 1 0 0 1 .853-.478h14.715a1 1 0 0 1 .871.508l7.345 13a1 1 0 0 1 0 .983l-7.345 13a1 1 0 0 1 -.871.509zm-14.11-2h13.526l6.78-12-6.78-12h-13.571l-6.756 11.039z"/><path d="m56.7 71a14 14 0 1 1 14-14 14.015 14.015 0 0 1 -14 14zm0-26a12 12 0 1 0 12 12 12.013 12.013 0 0 0 -12-12z"/></svg>
                        </div>
                    </div>
                </div>
                <div class="row flex-column align-items-center m-0">
                    <div class=" bordeEtiquC mt-4 mb-4">
                        <h4 class="text-center colorCuatro  p-2 m-1">DIVERSIDAD</h4>
                    </div>
                    <p class="textoVal">Consideramos que identificar la diversidad como principio resultará en la construcción de una comunidad íntegra e inclusiva</p>
                </div>
            </div>
        </div>
      
    </div>
</template>
<script>
export default {
    name:'NuestrosValors',
    data() {
        return {
            showU: false,
            showD:false,
            showT:false,
            showC:false
        }
    },
    methods:{
        agU(){
            this.showU = true
        },
        agD(){
            this.showD = true
        },
        agT(){
            this.showT = true
        },
        agC(){
            this.showC = true
        },
        quite(){
            this.showU = false,
            this.showD = false,
            this.showT = false,
            this.showC = false
        }
    }
}
</script>
<style>
    .nuestros{
        margin-left:3rem;
        font-size: 2.8rem;
        padding: 0;
        margin-bottom:4rem;
    }
    .contEmpoderamosTextoValores{
        max-width: 90%;
        margin:6rem auto 0 auto!important;
    }
    .nuestros::after{
        content: '';
        display: block;
        height: 0.2rem;
        width: 100%;
        background-color:   rgb(63, 63, 63);
        margin-left:-3.6rem;
    }
    .borderCIr{
        width: 10rem;
        height:10rem;
        border-radius: 100%;
        border: 2px #000 dashed
    }
    .denC{
        width: 9rem;
        height:9rem;
        border-radius: 100%;
    }
    .textoVal{
        font-size: 14px;
        text-align: justify;
        font-family: 'Kollektif';
    }
    .contValores{
        margin-top:6rem;
       
    }
    .colorUno{
        background: #14B71C;
        color:#fff;
    }
    .colorDos{
        background: #2359E5;
        color:#fff;
    }
    .colorTres{
        background: #FF3A12;
        color:#fff;
    }
    .colorCuatro{
        background: #f3e143;
        color:#fff;
    }
    .colorU{
        background: #14B71C;
        transition:all 0.3s ease;
    }
    .colorD{
        background: #2359E5;
        transition:all 0.3s ease;
    }
    .colorT{
        background: #FF3A12;
        transition:all 0.3s ease;
    }
    .colorC{
        background: #f3e143;
        transition:all 0.3s ease;
    }
    .bordeEtiquU{
        border:1px #14B71C solid;
    }
    .bordeEtiquD{
        border:1px #2359E5 solid;
    }    
    .bordeEtiquT{
        border:1px #FF3A12 solid;
    } 
    .bordeEtiquC{
        border:1px #f3e143 solid;
    } 
    .check{
        fill:none;
        stroke:#000;
        stroke-linecap:round;
        stroke-miterlimit:10;
        stroke-width:5px;
        }
    .icono-tas{
        width: 8rem;
    }
    .border-interno{
        border:solid 3px #000;
        border-radius:100%;
        width: 89%;
        height: 89%;
        display: flex;
        justify-content: center;
        align-items: center;
    }
@media only screen and (max-width: 600px){ 

}
@media only screen and (max-width: 768px) {

}
</style>